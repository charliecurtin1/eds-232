...
## trying with saved tracks, actually works
# get 300 saved tracks
for (i in seq(0, 280, 20)) {
  music <- get_my_saved_tracks(authorization = authorization_code,
                               offset = i) # get music
  features <- get_track_audio_features(ids = music$track.id) # then get audio features
  # somehow get the track name into the audio features before rbind 
  charlie_music <- rbind(charlie_music, features) %>% 
    bind_cols(music$track.name)
}

# converting music into audio features
audio_features <- data.frame()

for (id in list((1:100), (101:200), (201:300))) {
  features <- get_track_audio_features(charlie_music$track.id[id])
  charlie_features <- rbind(audio_features, features)
}
