...
- Our data appear to have two distinct groups where bushel sale price falls around 15-20 dollars per bushel and around 30-40 dollars per bushel. Additionally, pumpkins seem to be sold mostly on certain days of the year, with distinct regular gaps between sales.
...
- Variety is very highly negatively correlated with pumpkin price. As the value for variety decreases, the pumpkin price increases. The variety is in order as "FAIRYTALE", "MINIATURE", "MIXED HEIRLOOM VARIETIES", and "PIE TYPE". Since it's encoded numerically beginning with "FAIRYTALE" as 0, this likely means that "fairytale" pumpkins are the most expensive per bushel. 
...
Now that we have built a recipe, and actually confirmed that the data will be pre-processed appropriately, let's now build a regression model to answer the question: What price can I expect of a given pumpkin package?
...
- It seems that the model we estimated is not complex enough and is underfitting our test data, leading to high bias and low variance. We estimated pumpkin price on a number of encoded categorical features, while a linear regression might be better served to create predictions on continuous features. 
