...
```{r}
# load in the data, filter for habitat is demersal or habitat is reef-associated, binary encode the habitat types
...
ggplot(fish_clean, aes(as_factor(habitat))) + # order by count
  geom_bar(fill = "cornflowerblue") + # get counts
  theme_bw() + # bw theme
  labs(x = "Habitat",
...
```{r}
test_predict <- predict(object = train_fit, new_data = test) %>% 
  bind_cols(test)
